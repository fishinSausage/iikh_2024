...
public:
    ...
    void print(const std::string& filename = "groceryList.txt") const;    // txt 파일에 현재 식재료 목록을 저장.
    void load_from_txt(const std::string& filename);        // txt 파일에 저장된 식재료 목록을 불러옴.
    static std::vector<GroceryItem> items;
    static std::vector<Recipe> saved_recipes;
};
